...
http.createServer(function (request, response) {
			...
			return conn.createChannel().then(function (ch) {
				...
				ok = ok.then(function (_qok) {
					return ch.consume('281rabbitResponse', function (msg) {
						...
						console.log(" [x] Received 1 "+theFinalResponse);
						if (msg.content.toString() != null) {
							console.log(" [x] Received 2 '%s'", msg.content.toString());
							//temp
							...
							console.log(" [x] Received 3'%s'", msg.content.toString());
						}
					}, {noAck: true});
				});
	...
	request.on('end', function() {
		...
		rabbitReceiving()
		//console.log('out of rabbitReceiving');
		// rabbit code
		response.writeHead(200, {
			'Content-Type': 'text/plain',
			'Access-Control-Allow-Origin': '*',
		...
		//Note here we are listening to port 8081 while client runs on port 8080, hence in order to avoid Cross Domain Security Issue CORS is used.
		//Due to this reason, response header must contain Access-Control-Allow-Origin
		console.log("successfully got response here: " + theFinalResponse);
		response.end("Instance successfully created");
